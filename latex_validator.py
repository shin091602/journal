...
class LaTeXValidator:
    ...
    def check_common_latex_errors(self, filepath: str) -> List[Tuple[int, str, str, str]]:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if '\\\\' in line and not any(env in line for env in ['tabular', 'array', 'aligned']):
                if not re.search(r'\\begin\{(tabular|array|aligned)\}', line):
                    issues.append((
                        line_num,
                        "Formatting Warning",
